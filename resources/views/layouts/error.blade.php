...
<body>
    <div id="app">
        <nav class="d-print-none navbar navbar-expand-md navbar-light bg-white shadow-sm">
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <ul class="navbar-nav ms-auto">
                        ...
                        @php
                            // Verificar autenticación para páginas de error
                            $user = null;
                            $isAuthenticated = false;
                            
                            // Variable pasada desde el handler de excepciones
                            if (isset($currentUser) && $currentUser) {
                                $user = $currentUser;
                                $isAuthenticated = true;
                            }
                            // Verificar autenticación estándar
                            else {
                                try {
                                    if (auth()->check()) {
                                        $user = auth()->user();
                                    ...
                                    // Silenciar errores de autenticación en páginas de error
                                }
                            }
                        @endphp
                        ...
                        @else
                            ...
                            @if(auth()->user() && auth()->user()->role === 'admin')
                            <li>
                                <a class="nav-link" href="{{ route('ajustes.index') }}">{{ __('Ajustes') }}</a>
                            </li>
                            @endif
